...
spi_flash_direct_transfer_t transfer_write_enable = {.command = SPI_NOR_CMD_WREN,
						     .address = 0,
						     .data = 0,
						     .command_length = 1,
						     .address_length = 0,
...
spi_flash_direct_transfer_t transfer_read_status = {.command = SPI_NOR_CMD_RDSR,
						    .address = 0,
						    .data = 0,
						    .command_length = 1,
						    .address_length = 0,
						      ...
						      uint8_t desire_bit_value, uint32_t timeout)
{
	...
	while (timeout) {
		err = R_OSPI_B_DirectTransfer(p_ctrl, &transfer_read_status,
					      SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
		if (err != FSP_SUCCESS) {
			return -EIO;
		}

		if (((transfer_read_status.data & BIT(bit)) >> bit) == desire_bit_value) {
			break;
		}
...
static OSPI_B_RAMFUNC int flash_ospi_b_write_enable(ospi_b_instance_ctrl_t *p_ctrl)
{
	...
	err = R_OSPI_B_DirectTransfer(p_ctrl, &transfer_write_enable,
				      SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);
	if (err != FSP_SUCCESS) {
		return -EIO;
	}

	ret = flash_ospi_b_wait_operation(p_ctrl, 1, 1, MAX_TIME_WREN);
	if (ret != 0) {
							...
							size_t len)
{
	...
	while (len > 0) {
		...
		for (uint32_t i = 0; i < erase_time; i++) {
			...
			ret = flash_ospi_b_wait_operation(&ospi_b_data->ospi_b_ctrl, 0, 0,
							  MAX_TIME_ERASE);
			if (ret != 0) {
							...
							const void *data, size_t len)
{
	...
	while (len > 0) {
		...
		ret = flash_ospi_b_wait_operation(&ospi_b_data->ospi_b_ctrl, 0, 0, MAX_TIME_WRITE);
		if (ret != 0) {
				  ...
				  bool pattern_word_swapped)
{
	...
	if (memcmp((uint8_t *)calib_sector_address, &autocalibration_data,
		...
		ret = flash_ospi_b_wait_operation(&data->ospi_b_ctrl, 0, 0, MAX_TIME_ERASE);
		if (ret != 0) {
		...
		ret = flash_ospi_b_wait_operation(&data->ospi_b_ctrl, 0, 0, MAX_TIME_WRITE);
		if (ret != 0) {
...
static OSPI_B_RAMFUNC int flash_ospi_b_update_page_layout(struct flash_renesas_ra_ospi_b_data *data)
{
	...
	layout = k_malloc(sizeof(struct flash_pages_layout) * data->regions_layout.region_count);
	if (!layout) {
						       ...
						       const uint32_t *config_map)
{
	...
	regions = k_malloc(sizeof(struct flash_region_info) * region_count);
	if (!regions) {
						    ...
						    uint16_t enter_4byte_cmd)
{
	...
	if (enter_4byte_cmd != 0) {
		...
		ret = flash_ospi_b_write_enable(p_ctrl);
		if (ret != 0) {
			return ret;
		}
		transfer_4byte_enable.command = 0xB7;
						      ...
						      uint8_t address_length)
{
	...
	if ((sccr_len < 22) || !(sccr_buf[15] & BIT(31)) || !(sccr_buf[21] & BIT(31)) ||
	    (addr_enb_octa != addr_enb_dtr_octa)) {
		return -ENOTSUP;
	}
...
static OSPI_B_RAMFUNC int flash_ospi_b_update_flash_config(const struct device *dev)
{
	...
	int ret;
	fsp_err_t err;
	spi_flash_erase_command_t local_erase_list[JESD216_NUM_ERASE_TYPES] = {
		{.command = (bfp->dw8 >> 8) & 0xFF, .size = 1 << (bfp->dw8 & 0xFF)},
		{.command = (bfp->dw8 >> 24) & 0xFF, .size = 1 << ((bfp->dw8 >> 16) & 0xFF)},
		{.command = (bfp->dw9 >> 8) & 0xFF, .size = 1 << (bfp->dw9 & 0xFF)},
		{.command = (bfp->dw9 >> 24) & 0xFF, .size = 1 << ((bfp->dw9 >> 16) & 0xFF)}};
	...
	if (config->flash_size > MB(16)) {
		...
		if (init_addr_width == 24) {
			...
			ret = flash_ospi_b_4byte_enable(
				&data->ospi_b_ctrl, dw16.enter_4ba,
				config->special_require_info.enter_4byte_cmd);
			if (ret != 0) {
	...
	if (sfdp_region->param_available & SFDP_PARAM_SECTOR_MAP_AVAILABLE) {
		...
		regions = k_malloc(sizeof(struct flash_region_info));
		if (!regions) {
			LOG_ERR("Failed to allocate memory for flash regions");
			return -ENOMEM;
		}
	...
	if (target_mode == JESD216_MODE_8D8D8D) {
		if (sfdp_region->param_available & SFDP_PARAM_SCCR_MAP_AVAILABLE) {
			...
			ret = flash_ospi_b_sccr_8d_enable(&data->ospi_b_ctrl, sfdp_region->sccr_buf,
		...
		if (sfdp_region->pr1_buf[0] & JESD216_SFDP_PROFILE1_DW1_RDSR_DUMMY) {
			...
			transfer_read_status.dummy_cycles = 8;
		} else {
			data->ospi_b_command_set_table.status_dummy_cycles = 4;
			transfer_read_status.dummy_cycles = 4;
		}
...
static OSPI_B_RAMFUNC int flash_ospi_b_nor_probe(const struct device *dev)
{
	...
	sfdp_region->phdr =
		k_malloc((sfdp_region->header.nph + 1) * sizeof(struct jesd216_param_header));
	...
	sfdp_region->bfp_buf = k_malloc(sfdp_region->phdr[0].len_dw * sizeof(uint32_t));
	...
	for (uint8_t i = 1; i <= sfdp_region->header.nph; i++) {
		...
		case JESD216_SFDP_PARAM_ID_SECTOR_MAP:
			sfdp_region->smpt_buf =
				k_malloc(sfdp_region->phdr[i].len_dw * sizeof(uint32_t));
